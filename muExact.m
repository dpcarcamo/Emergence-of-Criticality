...
% 
%     if lambda < 0
%         Zcore = integ(@(psi) f(1i*psi));
%         Etanh = integ(@(psi) f(1i*psi) .* tanh_(1i*psi)) / Zcore;
%     end
% 
%     u = Etanh;
% end
...
function u = muExact(h, lambda, n)
    ...
    u = Etanh;  % may be complex for lambda<0; caller can take real(u) if desired
end
