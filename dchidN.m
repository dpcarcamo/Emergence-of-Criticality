function u = dchidN(h, lambda, n)
    ...
    Eftanh2 = integ(@(psi) f(psi) .* F(psi) .* (tanh_(psi)).^2) / Zcore;
    Etanh2  = integ(@(psi) f(psi).*(tanh_(psi).^2)) / Zcore;
    mu = muExact(h,lambda,n);

    u =Etanh2 + Etanh2.*Ef.*(n-1)- (n-1).*Eftanh2 - mu.^2 - 2.*n.*mu.*dmudN(h,lambda,n);  % may be complex for lambda<0; caller can take real(u) if desired
end
